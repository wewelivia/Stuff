...
def list_sources(api, regions: Optional[List[str]]) -> None:
    """
    Source codes present in the calendar, ranked by how many releases they own.

    Handy for working out which agencies feed your calendar.
    """
    releases = fetch_releases(api, regions, verbose=False)
    print(f"\n=== sources across {len(releases)} releases ===")
    tally: Dict[str, Dict[str, Any]] = {}
    for ent in releases:
        src = str(_meta(ent, "Source") or "(none)")
        entry = tally.setdefault(src, {"n": 0, "regions": set(), "example": ""})
        entry["n"] += 1
        reg = str(_meta(ent, "Region") or "")
        if reg:
            entry["regions"].add(reg)
        if not entry["example"]:
            entry["example"] = str(_meta(ent, "Description") or "")[:52]

    print(f"{'source':<24}{'releases':>9}  {'regions':<14} example")
    for src, e in sorted(tally.items(), key=lambda kv: -kv[1]["n"])[:60]:
        regs = ",".join(sorted(e["regions"]))[:13]
        print(f" {src:<23}{e['n']:>9}  {regs:<14} {e['example']}")
    print(f"\n{len(tally)} distinct sources.")
...
def run_scan(args) -> Dict[str, Any]:
    ...
    with open_client(args.client) as api:
        ...
        if len(todays) > args.max_releases:
            print(f"  capped at {args.max_releases} of {len(todays)} releases")
...
.desc .sub{display:block;color:var(--dim);font-size:11px;margin-top:2px;font-weight:400;}
...
.alert{color:var(--h);}
.widest{color:var(--dim);text-transform:uppercase;font-size:10px;letter-spacing:.05em;}
.sep{color:var(--dim);}
tr.grp>td.desc{font-weight:600;}
...
function groupRows(list){
  ...
  return order.map(function(k){
    ...
    // Only treat a member as the release's headline if it actually carries
    // aggregate wording. Plenty of releases have no aggregate at all: an
    // Irish wholesale price release is a list of commodities, and picking the
    // shortest name off it produced a row titled "Glass".
    g.trueHeadline = null;
    for(var i=0;i<g.members.length;i++){
      if(parseInt(g.members[i].tier,10) === 0){ g.trueHeadline = g.members[i]; break; }
    }
    g.lead = g.trueHeadline || g.worst;
    g.leadIsAggregate = !!g.trueHeadline;

    // With no aggregate to anchor on, rank order is arbitrary, so show the
    // members by how much they moved instead.
    if(!g.trueHeadline){
      g.members.sort(function(a,b){ return Math.abs(b.z) - Math.abs(a.z); });
    }

    // does anything other than the lead move?
    g.subMax = 0;
    g.members.forEach(function(r){
      if(r !== g.lead) g.subMax = Math.max(g.subMax, Math.abs(r.z));
    });
    return g;
  });
}
...
// A bare component name is useless on its own: "Glass" means nothing without
// "Wholesale Price Index" attached. Qualify it, unless the series description
// already repeats the release wording.
function qualified(r){
  var d = String(r.description||''), rel = String(r.release_description||'');
  if(!rel) return esc(d);
  var dl = d.toLowerCase(), rl = rel.toLowerCase();
  if(dl.indexOf(rl) !== -1 || rl.indexOf(dl) !== -1) return esc(d);
  return esc(rel) + '<span class="sep"> · </span>' + esc(d);
}
...
function drawGrouped(all){
  ...
  show.forEach(function(g, gi){
    var h = g.lead, opened = !!state.open[g.key];
    var flag = g.leadIsAggregate && g.subMax >= 2 && Math.abs(h.z) < 2;
    // The group is the release, so the release names the row. The series the
    // number belongs to goes underneath, because "Glass" on its own says
    // nothing without "Wholesale Price Index" above it.
    html += '<tr class="row grp '+band(g.worst.z)+'" data-g="'+gi+'">'
      ...
      + esc(g.desc || h.description)
      + '<span class="sub">'
      + (g.leadIsAggregate ? esc(h.description)
                           : '<span class="widest">widest of '+g.members.length
                             +':</span> '+esc(h.description))
      + (g.date? ' · '+esc(g.date):'')
      + (g.members.length>1 && g.leadIsAggregate? ' · '+g.members.length+' series':'')
...
def main() -> int:
    ...
    ap.add_argument("--list-sources", action="store_true",
                    help="print the source codes in your calendar with release counts, "
                         "then exit")
