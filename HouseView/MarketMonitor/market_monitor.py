...
the engine maths, API and HTML never change when the source does. Any
series that fails to fetch is reported in `errors` and excluded — never
fatal.
"""
...
def configure_proxy(proxy: str | None) -> None:
    """Optional outbound proxy for web mode (settings.proxy in the yaml)."""
    if proxy:
...
def fetch_bbg_batch(tickers: list[str], start: date,
                    host: str = "localhost", port: int = 8194
                    ) -> tuple[dict[str, list[tuple[str, float]]], list[str]]:
    """
    One HistoricalDataRequest for the whole universe (PX_LAST, daily).
    Returns ({ticker: [(date, value), ...]}, [error strings]).
    Requires the Bloomberg Terminal to be running (same setup as
    blpapi_provider.py — validate with diagnose_bbg.py / verify_universe.py).
    """
    import blpapi  # deferred import: web mode must not require it
    ...
    if not session.start():
        raise RuntimeError("Bloomberg session failed to start — is the Terminal running?")
    try:
        ...
        while True:
            ...
            for msg in ev:
                ...
                if sd.hasElement("securityError"):
                    ...
                    errors.append(f"{sec}: {err.getElementAsString('message')}")
                    continue
                ...
                for i in range(fd.numValues()):
                    ...
                    if row.hasElement("PX_LAST"):
                        ...
                        rows.append((f"{d.year:04d}-{d.month:02d}-{d.day:02d}",
...
def _normalise_units(cfg: dict, values: list[float]) -> list[float]:
    """
    Yield/OAS series are handled internally in percent. Sources disagree on
    quotation (USYC2Y10 and some OAS tickers quote in bp; FRED and most
    generic yields quote in percent), so detect bp-quoted series by the
    median absolute level and convert. Safe: no yield or OAS in percent
    terms plausibly exceeds 25 in absolute value.
    """
    if cfg.get("kind") not in ("yield", "oas") or not values:
        return values
...
def build_monitor(universe_path: str = UNIVERSE_FILE, use_cache: bool = True) -> dict:
    ...
    stats_list, errors = [], []
    mode = settings.get("data_source", "bbg")
    series_cfgs = uni.get("series", [])

    if mode == "bbg":
        bbg_cfgs = [c for c in series_cfgs if c.get("bbg")]
        tickers = [c["bbg"] for c in bbg_cfgs]
        try:
            bbg_data, bbg_errors = fetch_bbg_batch(
                tickers, start,
                host=settings.get("bbg_host", "localhost"),
                port=settings.get("bbg_port", 8194))
            errors.extend(bbg_errors)
        except Exception as exc:  # noqa: BLE001
            bbg_data = {}
            errors.append(f"Bloomberg fetch failed entirely: {exc}")
        for cfg in bbg_cfgs:
            data = bbg_data.get(cfg["bbg"], [])
            if not data:
                if not any(cfg["bbg"] in e for e in errors):
                    errors.append(f'{cfg["name"]} ({cfg["bbg"]}): no data returned')
                continue
    ...
    else:  # web mode: FRED + Stooq
        configure_proxy(settings.get("proxy"))
        for cfg in series_cfgs:
            if not cfg.get("source"):
                errors.append(f'{cfg["name"]}: no web source configured (bbg-only series)')
                continue
            data = []
